...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là bot của admin bot:)", "bạn kêu tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "Yêu anh ad nhất", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm em đau ~~~~", "Tuyển phi công nè ạ", "Cậu cô đơn ko?", "Được của ló :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm chồng em ko ạ?", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời", "Cút ra", "đứa nào spam bot ăn cc:)", "Yêu em ko?", "Chồng em đây rồi", "yêu admin của em hong, e bán nè:3"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Huy để được hỗ trợ\nLink fb nè: https://www.facebook.com/profile.php?id=100026348631060", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "@Mỹ Linh") || (event.body.toLowerCase() == "@Mỹ Linh")) {
    return api.sendMessage("Ai kêu tao đấy, mà làm ơn đừng tag bot mà hãy liên hệ admin qua Fb: https://www.facebook.com/profile.php?id=100026348631060 ", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao tên Đức Anh có gì dùng lệnh +admin list hoặc +ad để biết thêm chi tiết", threadID);
  };
