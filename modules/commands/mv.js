...
`, event.threadID, event.messageID);
  ...
  if (money >= 1000) {
  axios.get('https://api-vip.thanhnhu2.repl.co/videogaixinh').then(res => {
  ...
  let callback = function () {
    ...
    if (args[0] == "sex") {
      if (event.senderID != 100054769571636) return api.sendMessage(`[❗] Không phải mở NSFW rồi xem gì xem đâu =))`, event.threadID, event.messageID)
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://api-vip.thanhnhu2.repl.co/videosex').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
